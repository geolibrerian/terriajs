...
function generateLegend(legendHelper) {
    if (!defined(legendHelper.tableColumn) || (legendHelper.tableColumn.usesIndicesIntoUniqueValues)) {
        // If no table column is active, color it as if it were an ENUM with the maximum available colors.
        generateEnumLegend(legendHelper);
    } else {
...
function generateEnumLegend(legendHelper) {
    if (defined(legendHelper.tableColumn)) {
        legendHelper.tableColumn._isEnum = true;
        var colorBins = defaultValue(legendHelper.tableColumnStyle.colorBins, legendHelper.tableColumn.uniqueValues.length);
        legendHelper._binColors = buildEnumBinColors(legendHelper, colorBins);
    }
    var legendProps = buildEnumLegendProps(legendHelper);
...
function buildEnumBinColors(legendHelper, colorBins) {
    ...
    legendHelper._otherColor = defaultLargeEnumColorCodes[defaultLargeEnumColorCodes.length-1]; // Default "other" colour
    var binLookup = {};
    var i;
    if (!Array.isArray(colorBins)) {
        ...
        var colorCodes = ((binCount <= defaultEnumColorCodes.length) ? defaultEnumColorCodes : defaultLargeEnumColorCodes).slice(0, binCount);
        ...
        // Assign colours to valuesCount uniqueValues entries
        ...
        for (i = 0; i < colorBins.length; i++) {
            ...
            if (defined(bin.value)) {
                binLookup[bin.value] = bin.color;
            } else {
                legendHelper._otherColor = bin.color;
            }
        }
    }
    return binLookup;
...
function buildEnumLegendProps(legendHelper) {
    var tableColumn = legendHelper.tableColumn;
    var tableColumnStyle = legendHelper.tableColumnStyle;
    var binColors = legendHelper._binColors;

    if (!defined(tableColumn)) {
        // If legendHelper has a name use that, otherwise return undefined for no legend
        return defined(legendHelper.name) ? {
            items: [{
    ...
    if (legendHelper._cycleEnumValues) {
        items = [{
            ...
            multipleColors: uniqueValues.slice(0, )
        }];
    } else {
        items = [];
        var count = 0;
        for (var value in binColors) {
            if (Object.prototype.hasOwnProperty.call(binColors, value)) {
                items.push({
                    ...
                    color: binColors[value]
                });
                count++;
            }
        }
        if (tableColumn.uniqueValues.length > count) {
            items.push({
                title: tableColumn.uniqueValues.length - count + ' other values',
                color: legendHelper._otherColor
            });
        }
    }
...
LegendHelper.prototype.getColorArrayFromValue = function(value) {
    ...
    if (this.tableColumn._isEnum) {
        var val = this.tableColumn.uniqueValues[value];
        return getColorArrayFromCssColorString(Object.prototype.hasOwnProperty.call(this._binColors, val) ? this._binColors[val] : this._otherColor);
    }
