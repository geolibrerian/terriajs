...
function generateEnumLegend(legendHelper) {
    if (defined(legendHelper.tableColumn)) {
        ...
        if (!Array.isArray(colorBins)) {
            ...
            var colorCodes = (colorBins <= defaultEnumColorCodes.length) ? defaultEnumColorCodes : defaultLargeEnumColorCodes.slice(0, -1);
            for (var i = 0; i < colorCodes.length; i++) {
                binLookup[legendHelper.tableColumn.uniqueValues[i]] = colorCodes[i];
            }
            legendHelper.tableColumnStyle._otherColor = defaultLargeEnumColorCodes[defaultLargeEnumColorCodes.length-1];
        } else {
        ...
        legendHelper._binColors = binLookup;
    }
...
function buildEnumLegendProps(legendHelper) {
    ...
    if (legendHelper._cycleEnumValues) {
        ...
        for (var value in binColors) {
            if (binColors.hasOwnProperty(value)) {
                items.push({
        ...
        if (count < tableColumn.uniqueValues.length) {
            items.push({
                title: tableColumn.uniqueValues.length - count + ' other values',
                color: tableColumnStyle._otherColor
            });
        }
        items.reverse();
    }
...
LegendHelper.prototype.getColorArrayFromValue = function(value) {
    ...
    if (this.tableColumn._isEnum) {
        ...
        return getColorArrayFromCssColorString(this._binColors.hasOwnProperty(val) ? this._binColors[val] : this.tableColumnStyle._otherColor);
    }
